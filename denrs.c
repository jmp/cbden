...
void KMeans(PARTITIONING *pP, CODEBOOK *pCB, TRAININGSET *pTS, llong *distance,
    double *weight, int iter, int quietLevel, double time);
...
void PrintCentroidWeights(CODEBOOK *CB, double *weight);
void CheckOverflow(llong a, llong b);
void CheckClusterFreqs(CODEBOOK *pCB, PARTITIONING *pP);
...
int kmIter, int deterministic, int quietLevel, int useInitial, int monitoring)
{
  ...
  for (i=1; (i<=iter) && (!stop); i++)
    ...
    /* tuning new solution */
    LocalRepartition(&Pnew, &CBnew, pTS, weight, j, c, quietLevel);
    KMeans(&Pnew, &CBnew, pTS, distance, weight, kmIter, quietLevel, c);

    newError = ObjectiveFunction(&Pnew, &CBnew, pTS, weight);
    error    = CALC_MSE(newError);

    CheckClusterFreqs(&CBnew, &Pnew);
...
llong TotalDistance(TRAININGSET* TS, CODEBOOK* CB, PARTITIONING* P, int index)
{
  ...
  while (!EndOfPartition(i))
    {
    llong distance = VectorDistance(Vector(TS, i), Vector(CB, index),
...
double CalculateDensity(TRAININGSET* TS, CODEBOOK* CB, PARTITIONING* P, int index)
{
   ...
   */
  return CCFreq(P, index) / MeanDistance(TS, CB, P, index);
}
...
double *weight, int iter, int quietLevel, double time) 
{
  ...
  for (i = 0; i < iter; i++)
    ...
    OptimalPartition(pCB, pTS, pP, active, cdist, activeCount, distance, weight, quietLevel);
